...
$apiKey = trim(fread($APIKeyFile, filesize("../doNotPushToGit/polygonAPIKey")));
fclose($APIKeyFile);
$apiKey = trim($apiKey);
...
$channel->queue_declare('volatility_request',  false, true, false, false);
$channel->queue_declare('volatility_response', false, true, false, false);
...
$callback = function ($msg) use ($channel, $apiKey) {
    ...
    $channel->basic_publish($msgOut, '', 'volatility_response');
    echo "Responded with volatility for {$symbol}\n";
};

$channel->basic_consume('volatility_request', '', false, true, false, false, $callback);
