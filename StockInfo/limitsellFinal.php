...
$apiKey = trim(fread($APIKeyFile,filesize("../doNotPushToGit/finnhubAPIKey")));
fclose($APIKeyFile);
...
$channel->queue_declare('request_sell_limit_trade', false, true, false, false);
$channel->queue_declare('response_sell_limit_trade', false, true, false, false);
$channel->queue_declare('request_trade_execution',  false, true, false, false);

$sellLimitOrders = [];

echo "Sell Limit Consumer started...\n";

$callback = function ($msg) use (&$sellLimitOrders, $channel) {
    ...
    $channel->basic_publish($resp, '', 'response_sell_limit_trade');
};

$channel->basic_consume('request_sell_limit_trade', '', false, true, false, false, $callback);
